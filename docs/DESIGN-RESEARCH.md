...
- Arcane, Frost, and Rose accents share one neutral graphite GUI/HUD foundation and persist in `arcane-client.json`; none adds a green screen tint.
- Panels are near-opaque rather than translucent. A Click GUI that dims the world and then shows it through every surface is harder to read than one that commits: windows, bars, and tooltips sit at 0xFB–0xFF, and depth comes from a layered drop shadow, a top sheen, and a hairline edge instead of from letting the world bleed through.
- A module's state is carried by a toggle switch, an accent rail, and an accent label together, not by a background tint alone. Three coincident signals stay legible at any GUI scale and for players who cannot separate the enabled tint from the panel.
- Every animated surface moves through `UiAnimation.Track`, which eases on wall-clock time and adopts its target on its first frame. Opening the GUI is therefore still, and a switch travels the same arc at 30 and at 300 frames per second.
- Category glyphs are 7x7 bit masks drawn with plain fills. Header art that needs no texture, atlas slot, or resource lookup stays crisp at every GUI scale and cannot fail to load.
- Setting labels are trimmed against the space their own control leaves rather than against the window, so a long name is shortened instead of running under a switch, slider, or key pill.
