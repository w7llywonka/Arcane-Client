...
- `styles.css` — Krypton-inspired navy/steel visual system, responsive layouts, pills, cards, and motion.
...
The implementation is original Arcane code and content; it does not copy Krypton assets or source. Sora is loaded from Google Fonts with a system-font fallback. Replace the `https://discord.com/` fallback with the Arcane server invite when it is available.
