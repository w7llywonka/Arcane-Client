...
public final class RelogClientGameTest implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext context) {
        ...
        try (TestDedicatedServerContext server = context.worldBuilder().createServer(properties)) {
            server.connect();
            context.waitFor(client -> client.player != null && client.screen == null);
            ...
            context.waitFor(client -> client.level != null && client.player != null && client.screen == null, 5000);
            ...
            context.runOnClient(client -> {
                ArcaneClient.keybinds().relog().setBoundKey(InputConstants.Type.KEYSYM.getOrCreate(GLFW.GLFW_KEY_F10));
                KeyMapping.resetMapping();
                KeyMapping.click(InputConstants.Type.KEYSYM.getOrCreate(GLFW.GLFW_KEY_F10));
            });
            context.waitFor(client -> client.level == null && RelogController.statusLabel().equals("WAITING"));
            context.clickScreenButton("gui.cancel");
            context.runOnClient(client -> {
                require(!RelogController.isPending(), "cancel clears pending state");
                require(client.screen instanceof JoinMultiplayerScreen, "cancel returns to server list");
            });
            ...
            context.waitFor(client -> client.screen instanceof DisconnectedScreen && !RelogController.isPending(), 5000);
            long failedAt = System.nanoTime();
            context.waitFor(client -> System.nanoTime() - failedAt >= 2_200_000_000L, 5000);
            context.runOnClient(client -> require(client.screen instanceof DisconnectedScreen, "preserve failure screen"));
            ...
            context.runOnClient(client -> {
                ...
                ArcaneClient.keybinds().relog().setBoundKey(InputConstants.UNKNOWN);
                KeyMapping.resetMapping();
