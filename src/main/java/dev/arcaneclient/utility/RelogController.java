...
public final class RelogController {
    ...
    public static Result relog(Minecraft client) {
        ...
        if (server == null || server.ip == null || server.ip.isBlank() || server.isRealm()) {
            return Result.MULTIPLAYER_ONLY;
        }
        target = server;
    ...
    public static boolean tick(Minecraft client) {
        ...
        try {
            if (phase == Phase.LEAVE) {
                ...
                phase = Phase.WAIT;
                client.gui.setScreen(countdown);
            } else if (phase == Phase.WAIT) {
                if (client.gui.screen() != countdown || client.level != null) {
                    cancel();
                } else if (System.nanoTime() - disconnectedAt >= OFFLINE_DELAY_NANOS) {
                    ServerData server = target;
                    phase = Phase.CONNECT;
                ...
                if (client.level != null || client.gui.screen() instanceof DisconnectedScreen
            ...
            client.gui.setScreen(new DisconnectedScreen(new JoinMultiplayerScreen(new TitleScreen()),
                Component.literal("Relog failed"), Component.literal("Could not reconnect. Please join from the server list.")));
        }
        return isPending();
    }
    ...
    private static final class CountdownScreen extends Screen {
        private CountdownScreen() { super(Component.literal("Relog")); }
        ...
        public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
            ...
            context.centeredText(font,
                Component.literal("Reconnecting in " + seconds + "s…"), width / 2, height / 2 - 20, 0xFFFFFFFF);
        }
    }
